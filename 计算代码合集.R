...
list[1] #返回一个列表
list[[1]] #返回元素
list[["names"]] #选定名称为name的元素
list["names"] #返回列表
list[c("A", "B", "C")] #返回一个参数所决定的元素而构成的列表
list["names"] <- NULL #移除列表
unlist() #将列表转化为向量
listnames[sapply(listnames, is.null)] <- NULL #移除取值为空值的元素
listnames[listnames = o] <- NULL #使用条件来移除列表元素
...
rank() #返回在向量中的排名
#重复
rep(runif(1),5) #重复5次相同的数
rep(1:2, each = 2)
rep(1:2, each = 2, time = 2)
replicate(5,runif(1)) #重复5次runif(1) 不同的数
######apply family
group <- split(x,f) #x为向量，f为因子，向量分组，返回向量列表
