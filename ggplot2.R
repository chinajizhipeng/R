...
#修改默认属性
#scale_colour_brewer(palette = "Set1" ) #颜色模式
#scale_colour_manual(c("black","white"))#手动调颜色
#scale_shape_manual(c("black","white"))	#手动调形状 <可视化手册p65形状>
...
#自定义
ggplot(data,aes(x = data$a,y = data$b,colour = groupvar))+
  geom_point()+
  scale_fill_gradient(low="white",high="white") 	#颜色根据groupvar由白到黑
  
ggplot(data,aes(x = data$a,y = data$b,fill = groupvar)) + 
  geom_point(shape = 21,size = 2.5) +
  scale_fill_gradient(low = "black", high = "white",breaks = seq(70,170,by = 20),
                      guide = guide_legend()) 		#程度分为70-90-110...170
#两组（一组连续型变量，一组分类变量）
...
ggplot(data,aes(x = a,y = b)) + geom_point(alpha = .1) 						#1方法
ggplot(data,aes(x = a,y = b)) +
  stat_bin2d(bins = 50) +
  scale_fill_gradient(low = "lightblue",high = "red", limits = c(0, 6000)) 	#2方法自定义
...
ggplot(datanames,aes(x = a)) + 
  geom_histogram(binwidth = binsize, fill = "white",colour = "black",origin=31) #从x轴从31开始
...
####################注解
##添加文字注解
p + annotate("text",x = , y = , label = "GGG") #"text"为添加文本对象 xy为坐标
#指定文本属性
...
ggplot(datanames,aes(x = a, y = b)) + boxplot() + 
  scale_x_discrete(breaks = c("a", "b", "c"), 
                   labels = c("A", "B", "C")) +
  theme(axis.text.x = element_text(family = "Times", face = "italic",   #family是字体 italic是斜体
                                   colour = "darkred", size = rel(0.9))) #rel(0.9)当前大小字体的0.9倍
##修改坐标轴标签的文本
ggplot(datanames,aes(x = a, y = b)) + geom_point() + 
  xlab("A") + ylab("B\nC") 				#换行
ggplot(datanames,aes(x = a, y = b)) + geom_point()+
  scale_x_continuous(name = "hello")    #也行
##移除坐标轴标签
ggplot(datanames,aes(x = a, y = b)) + geom_boxplot() + 
...
#修改分面的文本标签，就是修改因子水平
#修改分面标签和标题的外观
p + theme(strip.text = element_text(face=,size=),                      		#标签颜色
		  strip.background=element_rect(fill="底色",colour="边框",size=1))  #标签背景
######################配色
library(RColorBrewer)
display.brewer.all() #brewer可用的颜色
#对离散型变量使用自定义调色板
#使用色盲友好的调色板
#对连续型变量使用自定义调色板
ggplot(data,aes(x=x1,y=y1,colour = z1))+
  scale_colour_gradient(low="white",hight="black")  #两种颜色渐变
  scale_colour_gradient2(low="",mid="",high="")		#三种颜色渐变
  scale_colour_gradientn(colours= = c("1","2","3","4","5")) #n中颜色渐变
#设定阴影颜色
#####################其他图形
##绘制相关矩阵图
##绘制函数曲线
